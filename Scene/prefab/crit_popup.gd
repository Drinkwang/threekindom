...
const SUIT_NAME_KEYS := ["民能载势", "谋士筹策", "商贾囤货", "士卒召列"]
...
func changeLanguage():
	...
	if currencelanguage=="ru":
		...
		_suit_label.add_theme_font_size_override("font_size",28)
...
func play(suit_index: int, desc: String = "", is_player: bool = true) -> void:
	...
	_suit_label.text = tr(SUIT_NAME_KEYS[suit_index]) if suit_index >= 0 and suit_index < SUIT_NAME_KEYS.size() else ""
