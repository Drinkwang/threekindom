...
const TASK_HIGHLIGHT_MARGIN := Vector2(4, 4)
...
#	_play_selected_slash()
	var original_pivot := general.pivot_offset
	var visual_center = general.head.position + general.head.size * 0.5
	_set_pivot_preserving_layout(general, visual_center)
	...
	tween.tween_callback(func():
		if is_instance_valid(general):
			_set_pivot_preserving_layout(general, original_pivot)
	)
