...
func _ready():
	...
	TooltipManager.register_tooltip(coin,tr("金币：持有的金钱数量(可购买道具)"))
	TooltipManager.register_tooltip(labor,tr("人口：拥有空闲人口(可转换为士兵)"))
	TooltipManager.register_tooltip(heart,tr("民心：当前百姓的支持度(<=0时游戏失败)"))
	#dd222test
